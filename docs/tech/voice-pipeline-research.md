...
结论:此类错误模型栈不可解,属产品交互层。解法(2026-07-03 已实现):
**追问式多轮合并消歧**——GPT 复核带证据强度(evidence: strong/weak,活体
验证:溪人名案例=weak,江泽民案例=strong);弱证据时应用主动追问「这个音有
好几个字,再告诉我一个用它的词」;下一句请求带上一轮 auditId,服务端取回
上一轮转写与新语音**合并判定**。真实录音端到端实证:余泽熙录音->溪(weak)
->追问答"康熙皇帝的熙"->合并判定 熙/康熙皇帝(1.4s)。追问语境一次性,
新问题由模型自行区分。

## 12. 全语料回测(2026-07-03,38 条真实录音)

判分规则:普通条目=字正确;人名类(单发不可判)=给出最优猜测且裁判标 weak
触发追问即通过。结果:**38/38(100%)**,同步 p50 1.0s / p90 1.5s。

回测揪出并修复:1) 静音幻觉——空录音被猜成"大"(few-shot 示例串味),修=
音频能量静音守卫(16-bit PCM 峰值+RMS 双阈值,真实语料标定:静音 rms<=42,
最弱真语音 rms=491,阈值 rms<150 && peak<2500);注意不能用"ASR 转写为空"
当守卫——超短真语音 ASR 也返回空但 omni 听得见(误杀实证)。2) 4 条
自动预填标签经 ASR 复听人工裁定纠正。教训:expected 自动预填=模型自评,
回测前必须人工核标;语料目录在 worktree/主仓各一份,回测要对准同一份。
