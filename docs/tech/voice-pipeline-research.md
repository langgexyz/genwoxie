...
eval:17/18(唯一 FAIL 为无语境裸同音字真歧义,任何裁判都不可判);单请求
延迟 6.5-11.5s(GPT 经 ccdirect 中转 3-9s 为大头);并发会放大中转排队,
儿童单用户场景为串行。ccdirect 的 GPT 文本 API 大陆可达且与理解服务同机,
是 GPT 能进运行时的前提。降延迟候选(未做):中转直连上游更快通道/裁判换
低延迟模型/预热连接。
