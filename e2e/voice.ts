...
// 场景七:真实鼠标按住 -> 拖出按钮边缘 -> 按钮外松手。浏览器抢手势曾把
// "在听"半路掐掉(滑出触发 pointerleave);指针捕获后滑出不算离开,松手在
// 哪都算松手(window pointerup)。dispatchEvent 不走命中测试,此场景必须真鼠标。
const box = await mic.boundingBox();
if (!box) throw new Error("mic button not visible");
await page.mouse.move(box.x + box.width / 2, box.y + box.height / 2);
await page.mouse.down();
await page.waitForSelector("#micBtn[data-recording]", { timeout: 10000 });
await page.mouse.move(box.x + box.width / 2, box.y - 120, { steps: 4 });
await page.waitForTimeout(300); // 给旧实现的 pointerleave 一个发作窗口,断言状态未被掐掉
const driftKeptListening = await page.evaluate(
  () =>
...
await page.mouse.up();
const releasedOutsideStops = await page
  .waitForFunction(
    () => !document.querySelector("#micBtn")?.hasAttribute("data-recording"),
    undefined,
    { timeout: 5000 },
  )
  .then(() => true)
